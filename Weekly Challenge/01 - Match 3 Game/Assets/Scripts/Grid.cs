    ...
    public bool FillStep()
    {
        ...
        for (int y = yDim - 2; y >= 0; y--)
        {
            for (int loopX = 0; loopX < xDim; loopX++)
            {
                ...
                GamePiece piece = pieces[loopX, y];

                if (piece.IsMovable())
                {
                    GamePiece pieceBelow = pieces[loopX, y + 1];

                    if (pieceBelow.Type == PieceType.EMPTY)
                    {
                        Destroy(pieceBelow.gameObject);
                        piece.MovableComponent.Move(loopX, y + 1, fillTime);
                        pieces[loopX, y + 1] = piece;
                        SpawnNewPiece(loopX, y, PieceType.EMPTY);
                        movedPiece = true;
                    }
                    else
                    {
